...
def main() -> None:
    ...
    with col2:           
        ...
        cl.draw_BigCard(str(st.session_state.Profile_Created_Date), str(st.session_state.Submission_Count), str(st.session_state.Contract_Count), str(st.session_state.YearsOfExperience), str(st.session_state.DaysWorked_Count), str(st.session_state.LastContractEnd_Datetime), str(st.session_state.Distance), str(st.session_state.DISCIPLINES), str(st.session_state.SPECIALTIES) \
                        , 'Profile Created Date'                  , 'Submission Count'                    , 'Contract Count'                    , 'Years Of Experience'                  , 'Total Days Worked'                   , 'Last Contract End'                           , 'Distance from Facility'          , "Disciplines"                    , "Specialty"                       \
                    )   
